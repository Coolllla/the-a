...
本 log 开头写「`LibraryV1.tsx` 现在是个**临时开发脚手架**……Extra 搬走时这个按钮和 `"use client"` 都要撤掉」——那是按 07-28 的旧决策理解的，**现在不成立**。

08-04 用户已经决定：番外**留在 `/library` 同一路由**，靠 state 在两个视图间切换，`change` 按钮是这个方案的正式承载。原因是他要的切换转场（鸽群飞过遮屏、幕下换视图）对换视图的时机有硬要求，跨路由的异步导航对不准那一帧。完整推导见 [07-27 log 决策五的 2026-08-04 再变更](./2026-07-27-library-skeleton.md)。

07-28 那条里的"单独做一个页面"只指**形态**，不指路由——番外页长得和时间轴完全不同这点没变。

### `--ring-cx` 调负之后暴露的耦合（已修）

用户要"圆再往左缩一点"，于是 `--ring-cx: 0px → -8rem`（起手值，视觉归他）。但**光改这一个值不够**：

`.content` 的 `left` 原来是 `calc(var(--ring-r) + var(--art-x))`，只看半径不看圆心偏移。圆心往左缩时环的右缘也跟着缩，而右半边不动 —— 中间会凭空多出一段空隙。现在补成 `calc(var(--ring-r) + var(--ring-cx) + var(--art-x))`，截图确认插画层跟着左移了 80px、间距仍是 `--art-x` 那 40px。

**教训**：把一组值声明成"旋钮"的时候，要检查所有派生位置是否都读了全部旋钮。这里的注释写着"改半径不用回来重调右半边"，字面没错，但漏掉了另一个旋钮也会动。

### 最短路公式：从纸上验算升级成穷举验证

上面「验证情况」里记的"最短路逻辑是纸上验算的"，现在穷举跑过了（`n = 2,3,4,5,6,7,8,12,15`）：任意 `(active → i)` 组合的跨度都 ≤ 半圈，落位全部正确。

一个边界值得知道：**偶数条目时最大跨度正好等于 `n / 2`**（`step > n / 2` 不成立所以不折），也就是正好半圈。这时两个方向等距，走哪边都对，不是 bug。

### 浏览器实测：静态确认了，转动过程还没

已截图确认的：环只露右半圈、选中项在 3 点钟、`.content` 那一整块拿到了样式（没有 `class="undefined"` 回归）、插画占位框和文字的实际位置。

还没实测的：**点击换节点时转动过程的观感**（是否真的走近路、0.55s 的时长和曲线合不合适）。静态截图看不出这个，需要人点。
